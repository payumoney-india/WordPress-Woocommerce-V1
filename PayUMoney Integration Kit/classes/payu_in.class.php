...
class WC_Gateway_Payu_In extends WC_Payment_Gateway {
	...
	*/
	...
	public function admin_options() {
    	...
    	<p><?php _e( 'PayUMoney works by sending the user to <a href="http://www.payu.in/">PayUMoney</a> to enter their payment information. Note that PayUMoney will only take payments in Indian Rupee.', 'woocommerce_payu_in' ); ?></p>
		<?php
			if ( get_option( 'woocommerce_currency' ) == 'INR' ) {
		?>
    ...
    function generate_payu_in_form($order_id) {
        ...
        $hash_data['udf2']				 = $hash_data['txnid'];
		...
		$payu_in_args = array(
			...
			// Merchant details
			'key'					=> $this->merchantid,
			'surl'					=> urlencode(add_query_arg( 'payu_in_callback', 1, $this->get_return_url( $order) )),
			'furl'					=> urlencode(add_query_arg(array('payu_in_callback' => 1, 'payu_in_status' => 'failed'), $this->get_return_url( $order))),
			'curl'					=> urlencode(add_query_arg(array('payu_in_callback' => 1, 'payu_in_status' => 'cancelled'), $this->get_return_url( $order))),
                        'service_provider'=>'payu_paisa', //paisa
			// Customer details
        ...
        $payuform .= '<input type="hidden" name="udf2" value="' . $hash_data['udf2'] . '" />' . "\n";
		//paisa
	...
	function process_payment($order_id) {
		...
		return array(
			'result' 	=> 'success',
			'redirect'	=> add_query_arg( 'order', $order->id, add_query_arg( 'key', $order->order_key, add_query_arg( 'pg', $this->get_post_var('pg'), get_permalink( get_option( 'woocommerce_pay_page_id' ) ) ) ) )
		);
	} // End process_payment()
